...
class MinimalPublisher(Node):
    ...
    def listener_callback(self, msg):
        ...
        self.relative_pose_1 = [ self.Mx1, self.My1, self.Phix1, self.Phiy1 ] # tensor data for MLP model
        self.relative_pose_2 = [ self.Mx2, self.My2, self.Phix2, self.Phiy2 ] # tensor data for MLP model
        self.relative_pose_3 = [ self.Mx3, self.My3, self.Phix3, self.Phiy3 ] # tensor data for MLP model
        self.relative_pose_4 = [ self.Mx4, self.My4, self.Phix4, self.Phiy4 ] # tensor data for MLP model
        self.relative_pose_5 = [ self.Mx5, self.My5, self.Phix5, self.Phiy5 ] # tensor data for MLP model
        self.relative_pose_6 = [ self.Mx6, self.My6, self.Phix6, self.Phiy6 ] # tensor data for MLP model
    
        u1_predicted = MLP_Model.predict(self.relative_pose_1, loaded_model) # predict control input u1, tensor
        u2_predicted = MLP_Model.predict(self.relative_pose_2, loaded_model) # predict control input u2, tensor
        u3_predicted = MLP_Model.predict(self.relative_pose_3, loaded_model) # predict control input u3, tensor
        u4_predicted = MLP_Model.predict(self.relative_pose_4, loaded_model) # predict control input u4, tensor
        u5_predicted = MLP_Model.predict(self.relative_pose_5, loaded_model) # predict control input u5, tensor
        u6_predicted = MLP_Model.predict(self.relative_pose_6, loaded_model) # predict control input u6, tensor
        ...
        " Calculate Mx's, My's and Phi's "  
        
        
        self.Phix1 = u2_predicted_np[0][0]   # 1x1
        self.Phiy1 = u2_predicted_np[1][0]   # 1x1
        
        self.Phix2 = ( u1_predicted_np[0][0] + u3_predicted_np[0][0] )/2 # 1x1
        self.Phiy2 = ( u1_predicted_np[1][0] + u3_predicted_np[1][0] )/2 # 1x1
        
        self.Phix3 = ( u2_predicted_np[0][0] + u4_predicted_np[0][0] )/2 # 1x1
        self.Phiy3 = ( u2_predicted_np[1][0] + u4_predicted_np[1][0] )/2 # 1x1
        
        self.Phix4 = ( u3_predicted_np[0][0] + u5_predicted_np[0][0] )/2 # 1x1
        self.Phiy4 = ( u3_predicted_np[1][0] + u5_predicted_np[1][0] )/2 # 1x1
        
        self.Phix5 = ( u4_predicted_np[0][0] + u6_predicted_np[0][0] )/2 # 1x1
        self.Phiy5 = ( u4_predicted_np[1][0] + u6_predicted_np[1][0] )/2 # 1x1
        
        self.Phix6 = u5_predicted_np[0][0] # 1x1
        self.Phiy6 = u5_predicted_np[1][0] # 1x1          
